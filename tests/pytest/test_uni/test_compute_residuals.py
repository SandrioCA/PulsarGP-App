...
def test_compute_residuals():
    ...
    logs_dir = os.path.abspath(os.path.join(config["paths"]["logs_dir"], "compute_residuals"))
    os.makedirs(logs_dir, exist_ok=True)

    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    log_path = os.path.join(logs_dir, f"log_compute_residuals_{timestamp}.txt")
    log_lines = []
    ...
    try:
        ...
        assert hasattr(residuals, "time_resids"), "El objeto no tiene atributo 'time_resids'."
        ...
        log_lines.append(f"Se calcularon {residuals.time_resids.shape[0]} residuos correctamente.")
        log_lines.append("La función compute_residuals pasó la prueba unitaria.")

    except Exception as e:
        log_lines.append("Fallo en test_compute_residuals:")
