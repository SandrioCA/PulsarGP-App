...
class App(ctk.CTkFrame):
    ...
    def guardar_archivo_txt(self):
        """
        Abre un diálogo "Guardar como..." y escribe los resultados completos del GP
        en un archivo .txt.
        """
        # Verificación del toggle
        if self.generate_file_switch.get() == 0:
            messagebox.showinfo("Función Desactivada", "Para guardar el archivo, activa primero el interruptor de '¿Generar archivo?'.")
            return

        # Verificación de que los cálculos se han realizado
        if not hasattr(self, 'tiempo_resultado') or self.tiempo_resultado is None:
            messagebox.showwarning("Sin Datos", "Primero debes ejecutar el proceso principal con el botón '►'.")
            return

        # Sugerir un nombre de archivo
        suggested_name = self.filename_entry.get()
        if not suggested_name:
            suggested_name = "resultados_gp_pulsar.txt"
        else:
        ...
        try:
            filepath = filedialog.asksaveasfilename(
                ...
                title="Guardar archivo de resultados completos",
                defaultextension=".txt",
                filetypes=(("Archivos de texto", "*.txt"), ("Todos los archivos", "*.*"))
            )

            # Si el usuario seleccionó una ruta, llamar al backend con todos los datos
            if filepath:
                be.crear_txt(
                    filepath=filepath,
                    tiempo=self.tiempo_resultado,
                    f=self.f_total,
                    f_err_up=self.f_err_up,
                    f_err_down=self.f_err_down,
                    df=self.df_total,
                    df_err_up=self.df_err_up,
                    df_err_down=self.df_err_down,
                    d2f=self.d2f_total,
                    d2f_err_up=self.d2f_err_up,
                    d2f_err_down=self.d2f_err_down
                )
                messagebox.showinfo("Éxito", f"Archivo de resultados completo guardado en:\n{filepath}")
...
# Dentro de tu clase App, reemplaza estos dos métodos completos
    ...
    def toggle_vista_grafico(self):
            """Alterna el gráfico incrustado entre residuos y el de 3 paneles."""
            ...
            try:
                if self.vista_actual == "residuos":
                    print("Cambiando a vista de Análisis GP (3 paneles)...")
                    # --- CORRECCIÓN: Se usan los nombres de variable correctos ---
                    fig = be.big_beautiful_graph_frecuencys(
                        self.clean_toas,  # <- MJD
                        self.f_total,     # <- Frecuencia
                        self.df_total,    # <- Derivada de Frecuencia
                        self.d2f_total    # <- Segunda Derivada
                    )
                    self._draw_plot(fig)
                    self.toggle_plot_button.configure(text="Ver Residuos")
                    self.vista_actual = "gp"
                
                else: # Si estamos viendo el GP, cambiamos al de residuos
    ...
    def abrir_graficos_en_ventanas(self):
        """
        Crea ventanas emergentes (Toplevel) para mostrar cada gráfico.
        """
        if not hasattr(self, 'model_object'):
            messagebox.showinfo("Información", "Primero debes ejecutar el proceso principal.")
            return
        ...
        try:
            ...
            # --- Generar y mostrar el primer gráfico (Residuos) ---
            unit_symbol = self.unit_map.get(self.selected_unit, "us")
            fig1 = be.plot_residuals(self.residuals_object, self.model_object, unit=unit_symbol)
            
            # Se crea la primera ventana emergente
            window1 = ctk.CTkToplevel(self)
            window1.title("Gráfico de Residuos")
            window1.geometry("800x600")
            
            # Se crea el lienzo y la barra de herramientas DENTRO de la nueva ventana
            canvas1 = FigureCanvasTkAgg(fig1, master=window1)
            toolbar1 = NavigationToolbar2Tk(canvas1, window1)
            toolbar1.update()
            canvas1.get_tk_widget().pack(side="top", fill="both", expand=True)

            # --- Generar y mostrar el segundo gráfico (3 paneles) ---
            fig2 = be.big_beautiful_graph_frecuencys(
                self.clean_toas,  # <- MJD
                self.f_total,     # <- Frecuencia
                self.df_total,    # <- Derivada de Frecuencia
                self.d2f_total    # <- Segunda Derivada
            ...
            # Se crea la segunda ventana emergente
            window2 = ctk.CTkToplevel(self)
            window2.title("Análisis de Frecuencia y Derivadas")
            window2.geometry("700x900") # Un poco más alta para los 3 paneles

            # Se crea el lienzo y la barra de herramientas
